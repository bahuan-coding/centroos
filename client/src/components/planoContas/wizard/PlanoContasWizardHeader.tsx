...
interface PlanoContasWizardHeaderProps {
  onClose: () => void;
}

export function PlanoContasWizardHeader({ onClose }: PlanoContasWizardHeaderProps) {
  const { isDirty, isSaving, lastSaved, currentStep, totalSteps } = usePlanoContasWizard();
  ...
  return (
    <header className="flex items-center justify-between gap-4 px-6 py-4 border-b border-zinc-100 bg-white">
      <div className="flex items-center gap-4">
        <div className="flex items-center justify-center w-10 h-10 rounded-xl text-white shadow-lg bg-gradient-to-br from-violet-500 to-indigo-600 shadow-violet-500/20">
        ...
        <div>
          ...
          <p className="text-xs text-zinc-500">
            Etapa {currentStep + 1} de {totalSteps} • Conforme ITG 2002
          </p>
        </div>
      </div>
