...
import { usePlanoContasWizard, Warning } from './PlanoContasWizardProvider';
import { cn } from '@/lib/utils';

export function PendenciasPanel() {
  const { warnings, goToStep, focusField, steps } = usePlanoContasWizard();
      ...
      <div className="flex items-start gap-3">
        ...
        <div className="flex-1">
          <p className="font-medium text-sm text-amber-800">
            {warnings.length} {warnings.length === 1 ? 'recomendação' : 'recomendações'}
          </p>
          <p className="text-xs text-amber-700 mt-0.5">
            A conta pode ser criada, mas recomendamos verificar estes pontos.
          </p>
