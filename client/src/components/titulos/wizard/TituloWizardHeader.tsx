import { X, Check, Clock, Receipt } from 'lucide-react';
import { Button } from '@/components/ui/button';
import { useTituloWizard } from './TituloWizardProvider';
import { cn } from '@/lib/utils';

interface TituloWizardHeaderProps {
  onClose: () => void;
}

export function TituloWizardHeader({ onClose }: TituloWizardHeaderProps) {
  const { isDirty, isSaving, lastSaved, currentStep, totalSteps, form } = useTituloWizard();
  ...
  return (
    <header className="flex items-center justify-between gap-4 px-6 py-4 border-b border-zinc-100 bg-white">
      <div className="flex items-center gap-4">
        <div className={cn(
          "flex items-center justify-center w-10 h-10 rounded-xl text-white shadow-lg transition-colors",
          form.tipo === 'receber' 
        ...
        <div>
          <h1 className="text-lg font-semibold text-zinc-900">
            Novo Título {form.tipo === 'receber' ? 'a Receber' : 'a Pagar'}
          </h1>
