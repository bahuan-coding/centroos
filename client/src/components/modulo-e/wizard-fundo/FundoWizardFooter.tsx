...
interface FundoWizardFooterProps {
  onSubmit: () => void;
}

export function FundoWizardFooter({ onSubmit }: FundoWizardFooterProps) {
  const { currentStep, totalSteps, goBack, goNext, isDirty, saveDraft, isSaving, isSubmitting, isEditMode } = useFundoWizard();
          ...
          >
            {isSubmitting ? (
              ...
              <>
                ...
                {isEditMode ? 'Salvar Alterações' : 'Criar Fundo'}
              </>
            )}
          </Button>
        ) : (
