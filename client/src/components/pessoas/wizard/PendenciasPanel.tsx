...
import { useWizard, Warning } from './WizardProvider';
import { cn } from '@/lib/utils';

export function PendenciasPanel() {
  const { warnings, goToStep, focusField, steps } = useWizard();
  ...
  const handlePendenciaClick = (warning: Warning) => {
    ...
    setTimeout(() => {
      if (warning.campo === 'documento') {
        focusField('doc_0');
      } else if (warning.campo === 'contato') {
        focusField('contato_0');
      } else if (warning.campo === 'endereco') {
        focusField('end_0_logradouro');
      }
    }, 100);
  };
      ...
      <div className="flex items-start gap-3">
        ...
        <div className="flex-1">
          ...
          <p className="text-xs text-amber-700 mt-0.5">
            O cadastro pode ser finalizado, mas recomendamos completar estes dados.
          </p>
