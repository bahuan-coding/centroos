...
import { FileText, Clock, List, X, Building2 } from 'lucide-react';
import { RascunhosList } from './RascunhosList';

export function RascunhoBanner() {
  const { hasPendingDraft, pendingDraft, loadDraft, discardCurrentDraft, drafts, steps } = useCentroWizard();
  ...
  const handleSelectDraft = (draft: CentroDraft) => {
    loadDraft(draft);
    setShowList(false);
  };
