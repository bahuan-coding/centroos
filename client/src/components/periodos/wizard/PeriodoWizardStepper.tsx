...
export function PeriodoWizardStepper() {
  const { steps, currentStep, goToStep, warnings, errors } = usePeriodoWizard();
      ...
      <nav className="hidden lg:flex flex-col w-64 shrink-0 bg-zinc-900 text-zinc-100" aria-label="Progresso do cadastro">
              ...
              <div key={step.id} className="relative">
                ...
                <button
                  ...
                  className={cn(
                    ...
                    status !== 'current' && status !== 'error' && isClickable && "hover:bg-white/5 cursor-pointer",
                ...
                >
                  <span className={cn(
                    ...
                    status === 'current' && "bg-gradient-to-br from-emerald-500 to-teal-600 text-white shadow-lg shadow-emerald-500/25",
        ...
        <div className="p-4 border-t border-zinc-800">
          <div className="p-3 rounded-lg bg-emerald-500/10 border border-emerald-500/20">
            <p className="text-[10px] text-emerald-300 uppercase tracking-wider font-medium">Regime</p>
            <p className="text-xs text-zinc-400 mt-1">Competência mensal — os lançamentos serão vinculados a este período</p>
          </div>
        </div>
      </nav>
      
      {/* Mobile: Top bar compacta */}
      <div className="lg:hidden px-4 py-3 border-b bg-zinc-50 shrink-0" aria-label="Progresso do cadastro">
