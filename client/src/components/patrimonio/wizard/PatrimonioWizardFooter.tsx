...
interface PatrimonioWizardFooterProps {
  onSubmit: () => void;
}

export function PatrimonioWizardFooter({ onSubmit }: PatrimonioWizardFooterProps) {
  const { currentStep, totalSteps, goBack, goNext, isDirty, saveDraft, isSaving, isSubmitting, isEditMode } = usePatrimonioWizard();
          ...
          >
            {isSubmitting ? (
              ...
              <>
                ...
                {isEditMode ? 'Salvar Alterações' : 'Cadastrar Bem'}
              </>
            )}
          </Button>
        ) : (
