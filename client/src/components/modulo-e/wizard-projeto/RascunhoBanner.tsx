...
import { FileText, Clock, List, X, Layers } from 'lucide-react';
import { Badge } from '@/components/ui/badge';
import { RascunhosList } from './RascunhosList';
import { cn } from '@/lib/utils';

export function RascunhoBanner() {
  const { hasPendingDraft, pendingDraft, loadDraft, discardCurrentDraft, drafts, steps, statusConfig } = useProjetoWizard();
  ...
  const handleSelectDraft = (draft: ProjetoDraft) => {
    loadDraft(draft);
    setShowList(false);
  };
  
  const stepLabel = steps[pendingDraft.currentStep]?.label || 'Início';
  const statusInfo = statusConfig[pendingDraft.status];
