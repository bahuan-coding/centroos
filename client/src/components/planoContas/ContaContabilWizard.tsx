...
import { ContaContabilWizardProvider, useContaContabilWizard } from './wizard/ContaContabilWizardProvider';
import { ContaContabilWizardHeader } from './wizard/ContaContabilWizardHeader';
import { ContaContabilWizardFooter } from './wizard/ContaContabilWizardFooter';
import { ContaContabilWizardStepper } from './wizard/ContaContabilWizardStepper';
...
interface ContaContabilWizardProps {
  open: boolean;
...
function WizardContent({ onClose, onSuccess }: { onClose: () => void; onSuccess?: (id: string) => void }) {
  const { currentStep, steps, isDirty, submit, saveDraft } = useContaContabilWizard();
  ...
  return (
    <>
      <div className="flex flex-col h-full bg-white">
        <ContaContabilWizardHeader onClose={handleClose} />
        
        <div className="flex flex-1 overflow-hidden">
          <ContaContabilWizardStepper />
        ...
        <ContaContabilWizardFooter onSubmit={handleSubmit} />
      </div>
...
export function ContaContabilWizard({ open, onOpenChange, onSuccess }: ContaContabilWizardProps) {
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent 
        ...
        aria-modal="true"
      >
        <ContaContabilWizardProvider onSuccess={onSuccess} onCancel={handleClose}>
          <WizardContent onClose={handleClose} onSuccess={onSuccess} />
        </ContaContabilWizardProvider>
      </DialogContent>
    </Dialog>
  );
}
